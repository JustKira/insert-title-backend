...
class PostingSerializer(serializers.ModelSerializer):
    class Meta:
        model = Posting
        fields = ('id', 'company', 'recruiter', 'image', 'description', 'creation_date',
                  'pay_range', 'location', 'num_applicants', 'position_title')


class CreatePostingSerializer(serializers.ModelSerializer):
    class Meta:
        model = Posting
        fields = ('company', 'position_title', 'recruiter',
                  'image', 'description', 'pay_range', 'location')
...
class UserSerializer(serializers.ModelSerializer):
    class Meta:
        model = ITUser
        fields = ('id', 'username', 'firstname', 'lastname', 'is_superuser', 'is_active', 'userType', 'phone_number', 'birthdate', 'bio',
                  'undergraduate_year', 'university', 'languages', 'github_link', 'linkedin_link', 'other_website_links', 'contact_links', 'is_company')
...
class StudentUserSerializer(serializers.ModelSerializer):
    class Meta:
        model = ITUser
        fields = ('phone_number', 'birthdate', 'bio', 'undergraduate_year', 'university',
                  'languages', 'github_link', 'linkedin_link', 'other_website_links')


class RecruiterUserSerializer(serializers.ModelSerializer):
    class Meta:
        model = ITUser
        fields = ('phone_number', 'birthdate', 'bio',
    ...
    def create(self, validated_data):
        user = ITUser.objects.create(
            username=validated_data['username'], email=validated_data[
                'email'], firstname=validated_data['firstname'], lastname=validated_data['lastname']
        )
...
class CVTemplateSerializer(serializers.ModelSerializer):
    class Meta:
        model = CV_Template
        fields = ['template_code', 'template_name', 'institutions',
                  'credit', 'required_data', 'numberOfUses']
